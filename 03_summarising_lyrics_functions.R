...
### reading and preparing dictionary  ----
dict = fread("/home/wojciech/Documents/text_mining/PoliMorf-0.6.7.tab", header = FALSE)

setnames(dict, c("V1", "V2"), c("word", "base"))
system.time({
  dict_lite = dict[, .(word = tolower(word), base = tolower(base))] %>% unique
})
system.time({
  setkey(dict_lite, word)
})
...
join_to_dict = function(dt) {
  ...
  dt_tmp = dict_lite[dt, .(word_id, word, base, freq = N)]
  dt_tmp[, base := coalesce(base, word)]
  
  # base forms in frequency (distinct words) order
  bases_all_sorted = dt_tmp[, .N, base][order(-N), base]
  ...
  dt_ded = dt_tmp[, .(uqN_forms = uniqueN(base),
                      freq = freq[1],
                      bases_list = factor(base, levels = bases_all_sorted) %>% sort(., na.last = F) %>% as.character %>% unique %>% list
  ),
  word
  ]
  ...
  dt_ded[ind_redup, .(word, uqN_forms, bases_list)]
}
...
fill_most_freq = function(dt) {
  dt[, most_freq := sapply(bases_list, . %>% head(., 1L))]
}
...
fill_least_freq = function(dt, verbose = TRUE) {
  ...
  # 1) for words that have only 1 base, pick it and go to 2), if there are none END
  # 2) remove already used base forms from the other words' bases list, then go to 1)
  fill_lf_iter = function(dt1) {
    if(verbose) cat("fill_lf_iter... ", iter)
    while(dt1[, any(uqN_forms==1)]) {
      counted_base_forms = dt[uqN_forms == 1, do.call(c, bases_list)]
      dt1[uqN_forms == 1, least_freq := sapply(bases_list, . %>% tail(., 1L))]
      
      # new base forms list and count
      dt1[, bases_list := sapply(bases_list, . %>% setdiff(., counted_base_forms))]
      dt1[, uqN_forms := sapply(bases_list, uniqueN)]
      if(verbose) cat("# ")
    }
    if(verbose) cat("\n")
  }
  
  # if there are no words with 1 base left, check if any have >1 
  #   for the first such word with least number of bases, pick one base (last one) and do away with the rest
  ...
  pick_random_form = function(dt1) {
    ...
    last_val = dt1[idx, unlist(bases_list)] %>% tail(1L)
    if(verbose) cat("pick_random_form: ", dt1[idx, word], " --> ", last_val, "\n")
    dt1[idx, `:=`(bases_list = list(last_val), uqN_forms = 1L)]
